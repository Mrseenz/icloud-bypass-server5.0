<?php
foreach (glob(__DIR__ . '/requests/*.xml') as $filename) {
    try {
        $activationinfo = file_get_contents($filename);

        $encodedrequest = new DOMDocument();
        $encodedrequest->loadXML($activationinfo);
        $activationDecoded = base64_decode($encodedrequest->getElementsByTagName('data')->item(0)->nodeValue, true);
        $fairPlayCertChain = $encodedrequest->getElementsByTagName('data')->item(1)->nodeValue;

        if ($activationDecoded === false) {
            throw new RuntimeException('Invalid base64 activation payload');
        }

        $decodedrequest = new DOMDocument();
        $decodedrequest->loadXML($activationDecoded);
        $nodes = $decodedrequest->getElementsByTagName('dict')->item(0)->getElementsByTagName('*');

        $deviceClass = '';
        $serialNumber = '';
        $deviceCertRequest = '';

        for ($i = 0; $i < $nodes->length - 1; $i += 2) {
            switch ($nodes->item($i)->nodeValue) {
                case 'DeviceCertRequest':
                    $decodedCsr = base64_decode($nodes->item($i + 1)->nodeValue, true);
                    $deviceCertRequest = $decodedCsr !== false ? $decodedCsr : '';
                    break;
                case 'SerialNumber':
                    $serialNumber = $nodes->item($i + 1)->nodeValue;
                    break;
                case 'DeviceClass':
                    $deviceClass = $nodes->item($i + 1)->nodeValue;
                    break;
            }
        }

        $devicefolder = __DIR__ . '/../devices/' . $deviceClass . '/' . $serialNumber . '/';
        if (!file_exists(dirname($devicefolder))) {
            mkdir(dirname($devicefolder), 0777, true);
        }
        if (!file_exists($devicefolder)) {
            mkdir($devicefolder, 0777, true);
        }

        $encodedrequest->save($devicefolder . 'device-request.xml');
        $decodedrequest->save($devicefolder . 'device-request-decoded.xml');
        file_put_contents($devicefolder . 'cert-request.crt', $deviceCertRequest);
        file_put_contents($devicefolder . 'fairPlayCertChain.crt', '-----BEGIN CERTIFICATE-----' . $fairPlayCertChain . '-----END CERTIFICATE-----');

        echo htmlspecialchars(basename($filename), ENT_QUOTES, 'UTF-8') . '............................ PROCESSED!<br/>';
    } catch (Exception $e) {
        echo 'Could not parse request file <b>' . htmlspecialchars(basename($filename), ENT_QUOTES, 'UTF-8') . '</b><br/>';
    }
}
?>
